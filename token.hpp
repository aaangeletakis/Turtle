...
auto lexFile(std::string& filedata, DocumentBucket& Document){
    ...
    for(;rit != rend; ++rit){
        ...
        case ';':{tmpNode.NodeFlags |= turtle::token::flag::DELIMITER_SEMICOLON;}break;
        ...
        case ',':{tmpNode.NodeFlags |= turtle::token::flag::DELIMITER_COMMA;}break;

        //Although I'm not going to program this in now cuz I don't use the '@', I'll classify it anyway
        case '@':{tmpNode.NodeFlags |= turtle::token::flag::DELIMITER_AT_SIGN;}break;
        case ':':{tmpNode.NodeFlags |= turtle::token::flag::DELIMITER_COLON;}break;
        case '.':{tmpNode.NodeFlags |= turtle::token::flag::DELIMITER_PERIOD;}break;

        //there is no '~=' operator in python so it is safe to put it in here
        case '~':{tmpNode.NodeFlags |= turtle::token::flag::ARITHMETIC_BIT_NOT;}break;

        case '0':
            if(token.size() >= 3){
                switch(token[1]){
                case 'x':{goto PUSH_BACK_LEX_TOKEN_GT;}break;
                case 'o':{goto PUSH_BACK_LEX_TOKEN_GT;}break;
                case 'b':{goto PUSH_BACK_LEX_TOKEN_GT;}break;
                case 'e':{goto PUSH_BACK_LEX_TOKEN_GT;}break;
                }
            }
            [[fallthrough]];
        case '1':
